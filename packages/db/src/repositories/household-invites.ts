import crypto from "node:crypto";
import { and, desc, eq } from "drizzle-orm";

import { decrypt, encrypt, hashToken, hmacIndex } from "@norish/config/crypto";
import { db } from "@norish/db/drizzle";

import { householdInvites } from "../schema";

/** Invites are valid for two weeks. */
const INVITE_TTL_MS = 14 * 24 * 60 * 60 * 1000;

export interface HouseholdInviteView {
  id: string;
  householdId: string;
  /** Decrypted for display to the admin who owns the household. */
  email: string;
  status: string;
  invitedByUserId: string | null;
  expiresAt: Date;
  createdAt: Date;
}

export interface HouseholdInviteRecord {
  id: string;
  householdId: string;
  status: string;
  expiresAt: Date;
  email: string;
}

function normalizeEmail(email: string): string {
  return email.trim().toLowerCase();
}

function toView(row: typeof householdInvites.$inferSelect): HouseholdInviteView {
  return {
    id: row.id,
    householdId: row.householdId,
    email: decrypt(row.emailEncrypted),
    status: row.status,
    invitedByUserId: row.invitedByUserId,
    expiresAt: row.expiresAt,
    createdAt: row.createdAt,
  };
}

/**
 * Create a pending email invite and return the raw token for the emailed link.
 * Any existing pending invite for the same email in this household is revoked
 * first, so "invite again" refreshes the token and expiry instead of piling up.
 */
export async function createHouseholdInvite(params: {
  householdId: string;
  email: string;
  invitedByUserId: string;
}): Promise<{ invite: HouseholdInviteView; token: string }> {
  const email = normalizeEmail(params.email);
  const token = crypto.randomBytes(24).toString("base64url");
  const expiresAt = new Date(Date.now() + INVITE_TTL_MS);

  await db
    .update(householdInvites)
    .set({ status: "revoked", updatedAt: new Date() })
    .where(
      and(
        eq(householdInvites.householdId, params.householdId),
        eq(householdInvites.emailIndex, hmacIndex(email)),
        eq(householdInvites.status, "pending")
      )
    );

  const [row] = await db
    .insert(householdInvites)
    .values({
      householdId: params.householdId,
      emailEncrypted: encrypt(email),
      emailIndex: hmacIndex(email),
      tokenHash: hashToken(token),
      invitedByUserId: params.invitedByUserId,
      expiresAt,
    })
    .returning();

  if (!row) {
    throw new Error("Failed to create household invite");
  }

  return { invite: toView(row), token };
}

/** Pending, unexpired invites for a household, newest first. */
export async function listPendingHouseholdInvites(
  householdId: string
): Promise<HouseholdInviteView[]> {
  const rows = await db
    .select()
    .from(householdInvites)
    .where(
      and(eq(householdInvites.householdId, householdId), eq(householdInvites.status, "pending"))
    )
    .orderBy(desc(householdInvites.createdAt));

  const now = Date.now();

  return rows.filter((row) => row.expiresAt.getTime() > now).map(toView);
}

/** Look up an invite by its raw token (from the emailed link). */
export async function getHouseholdInviteByToken(
  token: string
): Promise<HouseholdInviteRecord | null> {
  const [row] = await db
    .select()
    .from(householdInvites)
    .where(eq(householdInvites.tokenHash, hashToken(token)))
    .limit(1);

  if (!row) {
    return null;
  }

  return {
    id: row.id,
    householdId: row.householdId,
    status: row.status,
    expiresAt: row.expiresAt,
    email: decrypt(row.emailEncrypted),
  };
}

/** Revoke a pending invite (admin action). Returns true if a row changed. */
export async function revokeHouseholdInvite(
  householdId: string,
  inviteId: string
): Promise<boolean> {
  const result = await db
    .update(householdInvites)
    .set({ status: "revoked", updatedAt: new Date() })
    .where(
      and(
        eq(householdInvites.id, inviteId),
        eq(householdInvites.householdId, householdId),
        eq(householdInvites.status, "pending")
      )
    )
    .returning({ id: householdInvites.id });

  return result.length > 0;
}

/** Mark an invite accepted by a user. */
export async function markHouseholdInviteAccepted(inviteId: string, userId: string): Promise<void> {
  await db
    .update(householdInvites)
    .set({
      status: "accepted",
      acceptedByUserId: userId,
      acceptedAt: new Date(),
      updatedAt: new Date(),
    })
    .where(eq(householdInvites.id, inviteId));
}
