...
import { getObjectStore, keyFromWebUrl } from "./object-store";

/**
 * The persistent media store (filesystem or S3, per `STORAGE_DRIVER`). Object
 * keys are the tail of a stored web URL: `/recipes/{id}/{file}` ⇒ object key
 * `recipes/{id}/{file}`. Transient files used only for video transcoding stay
 * on the local disk via `fs` — they never reach the store.
 */
const objectStore = getObjectStore();

/** The object key for a stored web URL/prefix, joined with a filename. */
function mediaKey(webPrefix: string, fileName: string): string {
  return `${keyFromWebUrl(webPrefix)}/${fileName}`;
}
...
interface SaveImageOptions {
  /** Web URL prefix for the saved image (also the object-key prefix). */
  webPrefix: string;
...
async function saveImageBytesCore(bytes: Buffer, options: SaveImageOptions): Promise<string> {
  ...
  const key = mediaKey(options.webPrefix, fileName);

  // Content-addressed: identical bytes hash to the same name, so an existing
  // object is the same image — skip re-uploading it.
  if (!(await objectStore.exists(key))) {
    await objectStore.put(key, finalBytes, "image/jpeg");
  }
...
async function deleteImageByUrlCore(url: string, options: DeleteImageOptions): Promise<void> {
  ...
  try {
    // The web URL's tail is the object key (`/recipes/{id}/{file}` ⇒
    // `recipes/{id}/{file}`); a missing object is not an error.
    await objectStore.delete(keyFromWebUrl(url));
...
export async function readPublicAvatar(
  ...
  const object = await objectStore.get(`public-avatars/${filename}`);
...
export async function deleteRecipeImagesDir(recipeId: string): Promise<void> {
  try {
    await objectStore.deletePrefix(`recipes/${recipeId}`);
    log.info({ recipeId }, "Deleted recipe images");
  } catch (err) {
    log.warn({ err, recipeId }, "Could not delete recipe images");
  }
}
...
export async function deleteRecipeStepImagesDir(recipeId: string): Promise<void> {
  try {
    await objectStore.deletePrefix(`recipes/${recipeId}/steps`);
    log.info({ recipeId }, "Deleted step images");
  } catch (err) {
    // Ignore errors (nothing to delete)
    log.warn({ err, recipeId }, "Could not delete step images");
  }
}
...
export async function saveVideoFile(
  ...
  // The transcode output is a local temp file; move its bytes into the store.
  const bytes = await fs.readFile(sourcePath);

  await objectStore.put(key, bytes, videoMimeFromExt(ext));

  log.info({ sourcePath, key, size: stats.size }, "Video file saved");
...
export async function saveVideoBytes(
  ...
  await objectStore.put(`recipes/${recipeId}/${fileName}`, bytes, videoMimeFromExt(ext));
...
export async function deleteVideoByUrl(url: string): Promise<void> {
  ...
  try {
    await objectStore.delete(`recipes/${recipeId}/${filename}`);
